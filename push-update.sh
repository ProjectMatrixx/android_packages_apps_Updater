...
updates_dir=/data/matrixx_updates

if [ ! -f "$1" ]; then
   echo "Usage: $0 ZIP [UNVERIFIED]"
   echo "Push ZIP to $updates_dir and add it to Updater"
   echo
   echo "The name of ZIP is assumed to have matrixx-VERSION-DATE-TYPE-* as format"
...
# Assume matrixx-VERSION-DATE-TYPE-*.zip
...
adb shell "killall com.matrixx.updater 2>/dev/null"
adb shell "sqlite3 /data/data/com.matrixx.updater/databases/updates.db" \
